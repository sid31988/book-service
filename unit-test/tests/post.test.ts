...
describe("On successful POST request", () => {
    it("for valid data with releaseDate, it saves the Book record", async () => {
        let book = bookDtos[0];
    ...
    it("for valid data without releaseDate, it saves the Book record", async () => {
        let book = bookDtos[1];
        let mockedPgQuery = MockHelper.mockPgQuery({ rowCount: 1, rows: [] });
        let actual = await bookService.insert(book);
        let expected: BookDto = { ...book, resource: `${config.rest.baseUrl}/books/${book.isbn}` };
    ...
    it("for valid data with duplicate isbn, it saves the Book record", async () => {
        let book = bookDtos[0];
        let mockedPgQuery = MockHelper.mockPgQuery({ rowCount: 1, rows: [bookBos[0]] });
        let actual = await bookService.insert(book);
        let invalidResults = actual.validationResults.filter(x => x.fieldName === "Isbn" && x.type === ValidationType.Duplicate && !x.isValid);
        assert.deepStrictEqual(invalidResults.length > 0, true);
        mockedPgQuery.restore();
    });
});